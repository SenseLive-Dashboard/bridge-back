...
// Track clients and topic subscriptions
const clients = new Map();
const topics = new Map();

// Hash function for secure password storage
function hashPassword(password) {
    return crypto.createHash('sha256').update(password).digest('hex');
}

// SSL Certificate loading with validation
let privateKey, fullchain;
try {
    privateKey = fs.readFileSync('/etc/letsencrypt/live/senso.senselive.io/privkey.pem', 'utf8');
    fullchain = fs.readFileSync('/etc/letsencrypt/live/senso.senselive.io/fullchain.pem', 'utf8');
} catch (err) {
...
wss.on('connection', (socket) => {
    ...
    socket.on('message', (message) => {
        try {
            if (message.length > 5 * 1024 * 1024) {
                console.warn('Large message received and rejected:', message.length);
                socket.send(JSON.stringify({ type: 'error', message: 'Message size limit exceeded.' }));
                return;
            }
...
// Keep-alive setup
function setupKeepAlive(socket) {
    const interval = setInterval(() => {
